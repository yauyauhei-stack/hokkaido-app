...
itinerary = [
    ...
    ("DAY 7", "歸途：狸小路伴手禮", "09:00 狸小路商店街補貨 | 12:00 午餐 | 15:00 前往機場，帶著滿載的回憶返回香港。")
]

# 產生 HTML 區塊
sections_html = ""
for title, sub, desc in itinerary:
    # 圖片連結列表 (對應順序)
    imgs = [
    ...
    img = imgs[itinerary.index((title, sub, desc))]
    
    sections_html += f"""
        <div class="section" style="background-image: linear-gradient(rgba(0,0,0,0.3), rgba(0,0,0,0.3)), url('{img}');">
            <div class="text-box">
                <h1>{title}</h1>
...
<head>
    <style>
        ...
        /* 關鍵：移除磨砂玻璃，只保留純文字，帶有細微陰影以防背景太亮時看不清 */
